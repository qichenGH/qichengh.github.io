...
(function ($) {
	...
	var ci_frontend = {
	    ...
	    init: function () {
	        ...
	        var oldwindowwidth,
	            newwindowwidth,
				cfs = $('html').css('font-size').replace(/[^-\d\.]/g, '');/* cfs = currentfontsize, calulating this so breakpoints are the same as in css (screen width in rem's), BvL */

	        ci_frontend.setup_js();
	        ci_frontend.setup_afontgarde();
			ci_frontend.setup_videoposter();
			ci_frontend.setup_generalnav();
	        ci_frontend.setup_distributeheightswithinrow();
	        ci_frontend.setup_oneclick();
			ci_frontend.setup_smoothscroll();
			ci_frontend.setup_promotype02();

	        if($(window).width() <= (cfs*48)) {/* specific function for viewport smaller or equal to 48rem/768px (eg ipad portrait), BvL */
				ci_frontend.setup_mobilenav();

	        } else { /* specific function for viewport width greater then 48rem/768px), BvL */
				ci_frontend.setup_nav();
	        }
	        ...
	        $(window).resize(function () {
	            ...
	            waitForFinalEvent(function (){
	                ...
	                if(oldwindowwidth != newwindowwidth){/* check if the width of the viewport has changed, Bvl */

	                    if((newwindowwidth <= (cfs*48) /*768*/) && (oldwindowwidth > (cfs*48))) {/* when the window is resized from wider then 768 to smaller or equal to 768 (768px is viewport of ipad portrait), BvL */
							ci_frontend.setup_mobilenav();
	                    }
	                    if((newwindowwidth > (cfs*48)) && (oldwindowwidth <= (cfs*48))) {/* when the window is resized from smaller or equal to 768 to wider then 768 (768px is viewport of ipad portrait), BvL */
							ci_frontend.setup_nav();
							location.reload();
	                    }
	    ...
	    setup_videoposter: function(){// simple 'hide' function, BvL
	        "use strict";

			$('#main-page-content picture.video-poster').css({'z-index': '2', 'display': 'block'});
			$('#main-page-content picture.video-poster').append('<button class="poster-hide" id="poster-hide"><span class="icon icon_play" aria-hidden="true"></span></button>');

			$('#main-page-content picture.video-poster button').on( 'click touch', function() {
				$(this).css('z-index', '-1');
				$(this).parent('picture.video-poster').css('z-index', '-1');
			});
	    },
	    setup_afontgarde: function () {// Afontgarde is a plugin that registers if fonts are loaded, BvL
	        "use strict";
	    ...
	    setup_generalnav: function() {// several navigation elements will be set up here, used in both desktop and mbile navigation, BvL
	        "use strict";

	        if(!$('#menu').length > 0){// insert the menu (buttons) when it isn't already there, BvL
		        $('#container').prepend('<ul class="menu" id="menu"><li><button class="nav-toggle" id="nav-toggle"><span class="icon icon_menu" aria-hidden="true"></span>Menu</button></li><li><button class="search-open" id="search-open"><span class="icon icon_magglass" aria-hidden="true"></span><strong class="icon-fallback">Search</strong></button></li></ul>');
		    }
	        if(!$('#search-close').length > 0){// insert the search close button when it isn't already there, BvL
				$('#search-form').prepend('<button class="search-close" id="search-close" href="/search.php"><span class="icon icon_close" aria-hidden="true"></span><strong class="icon-fallback">close search panel</strong></button>');
			}
			$('#primary-nav ul.level_1 > li > a').on( 'click touch', function() {
				var parentli = $(this).parent('li:first'),
					siblings = $(this).parent('li:first').siblings();

				siblings.removeClass('selected'); // on click of level 2nav all other nav2 elements need selected removed, BvL

				if(parentli.hasClass('selected')){// toggle 'selected' and add focus, BvL
					parentli.removeClass('selected');
					parentli.find('a:first').blur();
				}else{
	    ...
	    setup_nav: function() {
			...
			$('#nav-wrapper .search_supplimental-nav_wrapper').show();
			$('#nav-wrapper #search-form').hide();

			$('#menu li button, .front .accessible-nav li:first a').on( 'click touch', function() {// on the homepage the two buttons reveal the menu, push down the content, BvL
				$('#nav-wrapper').css({'display': 'block'}).animate({ 'marginTop': '0'}).addClass('open');
				$('#homepage-top p.logo').hide();

				$('#nav-wrapper #home-link').attr('tabindex','0').css('outline','0').focus();

				if ($(this).hasClass('search-open')) {
					$('#nav-wrapper #supplimental-nav').hide();
					$('#nav-wrapper #search-form').show();
					$('#nav-wrapper #search-form input[type=text]').focus();
				}
				return false;
			});

			$('#supplimental-nav #search-open').on( 'click touch', function() {
				$('#nav-wrapper #search-form').show();
				$('#nav-wrapper #supplimental-nav').hide();
				$('#nav-wrapper #search-form input[type=text]').focus();
				return false;
			});
			$('#nav-wrapper #search-close').on( 'click touch', function() {
				$('#nav-wrapper #search-form').hide();
				$('#nav-wrapper #supplimental-nav').show();
				$('#nav-wrapper #search-form input[type=text]').blur();
			});

		},

	    setup_mobilenav: function(){
	        "use strict";

			$('#nav-wrapper').css({'display': 'block', 'margin-top': '0'});
			$('#nav-wrapper nav').hide();
			$('#nav-wrapper .search_supplimental-nav_wrapper').hide();


			$('#menu li #nav-toggle').on( 'click touch', function() {
				$(this).toggleClass('open');
				$(this).parents('ul:first').find('#search-open').removeClass('open');
				$('.search_supplimental-nav_wrapper').hide();
				...
				$('#primary-nav ul.level_1 > li:first').focus();

				return false;
			});

			$('#menu li #search-open').on( 'click touch', function() {
				$(this).toggleClass('open');
				$(this).parents('ul:first').find('#nav-toggle').removeClass('open');
				$('#nav-wrapper nav').hide();
	...
	setup_promotype02: function() {
        "use strict";
        var cfs = $('html').css('font-size').replace(/[^-\d\.]/g, '');/* cfs = currentfontsize, calulating this so breakpoints are the same as in css (screen width in rem's), BvL */


        if($(window).width() <= (cfs*48)) {/* specific function for viewport smaller or equal to 48rem/768px (eg ipad portrait), BvL */
		        $('.promo-type02 .inner2').show();
				$('.promo-type02 .inner1').hide();

        } else { /* specific function for viewport width greater then 48rem/768px), BvL */
			$('.promo-type02').css({'cursor':'pointer'});
	        $('.promo-type02 .inner1').show();
			$('.promo-type02 .inner2').css({'z-index': '-1', 'opacity':'0'}).attr('tabindex','-1');
			$('.promo-type02 .inner2 a').attr('tabindex','-1');

	        $('.promo-type02').on( 'mouseenter', function() {
				$(this).find('.inner1').css({'z-index': '-1', 'opacity':'0'});
				$(this).find('.inner2').attr('tabindex','0').css({'z-index': '0','outline':'0', 'opacity': '1'});
				$(this).find('.inner2 a').attr('tabindex','0');

	        });
   	        $('.promo-type02').on( 'mouseleave', function() {
				$(this).find('.inner1').attr('tabindex','0').css({'z-index': '0','outline':'0', 'opacity': '1'});
				$(this).find('.inner1 a').attr('tabindex','0');
				$(this).find('.inner2').css({'z-index': '-1', 'opacity': '0'}).attr('tabindex','-1');
				$(this).find('.inner2 a').attr('tabindex','-1');
			});

			$('.promo-type02 .inner1 a').on( 'focus', function() {
				$(this).parents('.promo-type02').find('.inner2').attr('tabindex','0').css({'z-index': '0','outline':'0', 'opacity': '1'});
				$(this).parents('.promo-type02').find('.inner1').css({'z-index': '-1','outline':'0', 'opacity': '0'});
			});

			/*$('.promo-type02 a').on( 'blur', function() {
				$(this).parents('.promo-type02').find('.inner1').attr('tabindex','0').css({'z-index': '0','outline':'0', 'opacity': '1'});
				$(this).parents('.promo-type02').find('.inner2').css({'z-index': '-1','outline':'0', 'opacity': '0'});
			});*/
	    }
	},
	    setup_smoothscroll: function () {// all acnhor links get a smooth scroll to where ever they link, BvL
		    "use strict";
	        ...
	        $(document).on("click", 'a[href^="#"]', function (event){
	            ...
	            if (anchor.length) {
	                ...
	                anchor.attr('tabindex','0').css('outline','0').focus();
	            }
	        });
	    },

		setup_distributeheightswithinrow: function (){// cutom function to give all elements within the same row the same height, based on the hightest within that row, Bvl
		    "use strict";
			...
			var		t = 1,
					...
					highestarray = [],
					previous,
					size  = ($('.distributeheightswithinrow').length)-1;

			$('.distributeheightswithinrow').css({'min-height': ''});

			$('.distributeheightswithinrow').each(function(i){
				var 	dis = $(this),
						disheight = dis.outerHeight();

				dis.removeClass (function(index,css) {
				    return (css.match(/\bpartofrow-\S+/g) || []).join(' ');
				});

				if(i ===0){//first match, highest is obviousely the height of the only match so far, BvL
					highest = disheight;
				}else {
					if (dis.offset().top != previous.offset().top){
						highestarray.push(highest);
						highest = disheight;
						t= t+1;
					}else{
						if (disheight > highest){
							highest = disheight;
						}
					}
				}

				dis.addClass('partofrow-'+t);
				previous = dis;
			...
			$('.distributeheightswithinrow').each(function(){
				var dis = $(this),
					num = dis.attr('class').match(/partofrow-(\d+)/)[1];

				if(dis.css('display')=='table') {
					dis.css('height', highestarray[num-1]);
				}else{
					dis.css('min-height', highestarray[num-1]);
				}
			});

		},
	    setup_oneclick: function () {
	        "use strict";

	        $('.one-click').each(function (){// clicking on '.one-click' elements will activate the link inide the block (especially done for touch devices), BvL
	            var linky = $(this).find('a[href]:not([href=""],[href="#"])');//look for links with a (not empty) href and not an anchor, BvL
	            var oneclick_element = $(this);
	            if(linky.length !== 0) {//if there is a link within '.one-click' , BvL

	                if(linky.length == 1) {// if there is only one link then that 'll be used for the overall link, BvL
	                    oneclick_element.css('cursor','pointer');
	                    oneclick_element.hover(function (){ $(this).toggleClass('over'); });
	                    oneclick_element.click(function (){ window.location = linky.attr("href"); return false; });
	                }else{//if there are more then one links in the 'one-click' element, find the faux-button (Read more) one, BvL
	                    linky.each(function (){
	                        var linx = $(this);
	                        if (linx.parent('p.faux-button')) {
	                            oneclick_element.css('cursor','pointer');
	                            oneclick_element.hover(function (){ $(this).toggleClass('over'); });
	                            oneclick_element.click(function (){ window.location = linx.attr("href"); return false; });
	                        }
	                    });
	                }
	            }
  ...
  $(document).ready(function(){
    ...
    if ( typeof ci_frontend !== 'undefined' ) {
      ci_frontend.init();
    }

    // Automatically add lightbox functionality to wysiwyg images
    $('.image-wrapper a').each(function() {
    	var lightbox = $(this).attr("data-lightbox");
	    if(lightbox === undefined) {
	    	$(this).attr('data-lightbox', 'callaghan');
	    	$(this).append('<span class="icon icon_expand"><span class="path1"></span><span class="path2"></span><span class="path3"></span></span><strong class="icon-fallback">open this image in new window: </strong>');
	  	}
	  });
    ...
    // Add markup for inline documents
    $('span.file').each(function() {
    	$(this).find('a').addClass("file-download");

    	var meta = $(this).find('a').attr('type');
    	var size = meta.split("=");
    	var type = size[0].split(/\/|;/);
    	var filetype = type[1].toUpperCase();
    	//alert(filetype);
    	if(filetype == 'MSWORD') {
    		filetype = 'DOC'
    	}
    	...
    	var filesize = humanFileSize(size[1], 'true');
    	$(this).find('a').append('<span class="file-size"> (' + filetype + ', ' + filesize +')</span>');
    	// Remove file icon
    ...
    // Custom comment validation
		...
		jQuery.validator.addMethod("validateComment", function( value, element ) {
    ...
    $("#comment-form").validate({
        ...
        messages: {
          captcha_response: "Please enter the characters shown"
        }
    });
    $('iframe').wrap('<div class="media-holder"><div class="content"></div></div>');
    
    $('.promo-type02, .promo-type04').bind('click', function(event) {
      if( !$( event.target ).is( "a" ) ) {
        var target = $(this).find('a').attr('target');
        if (target == '_blank') {
          // Open new tab/window to the url from this element inside it
...
/** This code was taken from https://codepen.io/digistate/pen/MvapbE **/
(function ($) {
  $(document).ready(function() {
    var slideWrapper = $(".field-carousel-items-wrapper"),
    ...
    function playPauseVideo(slick, control){
      ...
      slideType = currentSlide.find('video').length === 0 ? 'image' : 'video';
      player = currentSlide.find("iframe").get(0);
      startTime = currentSlide.data("video-start");

      // YouTube/Vimeo functionality removed as out-of-scope, and changes below
    ...
    function resizePlayer(iframes, ratio) {
      if (!iframes[0]) return;
      var win = $(".field-carousel-items-wrapper"),
        width = win.width(),
        playerWidth,
        height = win.height(),
        playerHeight,
    ...
    $(function() {
      ...
      slideWrapper.on("init", function(slick){
        ...
        $('video', slick).bind('ended', function() {
          slick.slick('slickPlay');
...
(function($) {
  $(function() {
    ...
    $('.field-section-carousel-items-wrapper').slick({
      dots: true,
      autoplay: true,
      autoplaySpeed: 5000,
    });

    $('.contact-block-text').matchHeight();

    $('#back-link').on('vclick', function(e) {
      if (document.referrer != '') {
        e.preventDefault();
        history.go(-1);
      }
    });
    $('button.print').on('vclick', function(e) {
      window.print();
    });
    // Apply selectize to grants exposed filter dropdown.
    $('#views-exposed-form-grants-page select').selectize({
      select: true,
      sortField: 'text'
    });
    $('.selectize-input input').prop('readonly', 'readonly');

    // Hide sidebars on news/events/blogs page depending on selected
    // tab.
    if ($('#quicktabs-news_landing').length) {
      var currentTab = $('#quicktabs-news_landing > .filter li.active a')
        .attr('id');
      showHideSidebars(currentTab);
      function showHideSidebars(currentTab) {
        var id = currentTab;
        if (id == 'quicktabs-tab-news_landing-0'
            || id == 'quicktabs-tab-news_landing-1'
            || id == 'quicktabs-tab-news_landing-2') {
          $('#regionright').hide();
          $('#news-sidebar').show();
        } else {
          $('#regionright').show();
      ...
      $('#quicktabs-news_landing > .filter a').on('click', function() {
        var id = this.id;
        showHideSidebars(id);
      })
    }

    // Hide/show close button on date picker fields.
    $(document).on('mousedown', function(e) {
      var element = $('#edit-field-grant-dates-value-value-datepicker-popup-0, #edit-field-grant-dates-value-1-value-datepicker-popup-0');
      if (!element.is(e.target)) {
        element.removeClass('close');
      }
    });
    $('#edit-field-grant-dates-value-value-datepicker-popup-0, #edit-field-grant-dates-value-1-value-datepicker-popup-0')
      .on('vmousedown', function() {

      $('#ui-datepicker-div').toggle();
      if ($(this).attr('id') == 'edit-field-grant-dates-value-value-datepicker-popup-0') {
        $('#edit-field-grant-dates-value-1-value-datepicker-popup-0')
          .removeClass('close');
      }
      else {
    ...
    $('#edit-field-grant-dates-value-value label').remove();
    $('#edit-field-grant-dates-value-1-value label').remove();
    if ($('body.section-batch.page-batch').length) {
      history.replaceState({}, 'Grant Recipients', document.referrer);
    }
    $('#edit-reset').on('vclick', function(e) {
    ...
    $('.filter-button').click(function() {
      ...
      $('#regionright').toggleClass('rightRegionVisible');

      var filterDiv = $('#regionright').children('div');

      filterDiv.toggleClass('rightRegionVisibleHeaders');
      filterDiv.find('h2').click(function() {
        $(this).parent('div').toggleClass('filterContainerToggle');
        $(this).toggleClass('rightFilterSelected');
        $(this).parent('div').find('.view').toggleClass('rightFiltersVisibile');
        $(this).parent('div').find('.mailchimp-signup-subscribe-form')
          .toggleClass('rightFiltersVisibile');
      });
    });
    $('.comment-form').find('.form-item')
      .not('.form-item-captcha-response').each(function(){

      var formText = $(this).find('label').text().trim();
      $(this).find('input').attr('value', formText);
      $(this).find('textarea').text(formText);
    });

    var inputComment = $('.comment-form').find('.form-item')
      .not('.form-item-captcha-response');

    $(inputComment).find('input').focus(function() {
      var inputText = $(this).prev().text().trim();
    ...
    $(inputComment).find('textarea').focus(function() {
      var inputText2 = $(this).parent().prev().text();
      if ($(this).text() == inputText2) {
        $(this).text('');
      }
      $(inputComment).find('textarea').blur(function() {
        if ($(this).text() == '') {
          $(this).text(inputText2);
        }
      });

    });

    // Prevent name label from being submitted as value

    $('#comment-form').submit(function() {
      if ($('#edit-name').val() == 'Your name') {
        $('#edit-name').val('');
      }
    });
    ...
    window.addEventListener('load', function() {
      /* Add Captions */
      $('.blogleft').find('p img').each(function() {
        // Width of image
        var maxWidth = $(this).width();
        // if does not have captioned class...
        if (!$(this).hasClass('captioned')
            && !$(this).hasClass('file-icon')) {
          // if this image contains a title
          if ($(this).attr('title')) {
            // set caption string as image title
            var imgtitle = $(this).attr('title').trim();
            // check image float value
            var floatValue = $(this).css('float');
            // if caption exists
            if (imgtitle != null || imgtitle != '') {
              $(this).wrap('<div style="max-width:' + maxWidth
                      + 'px" class="captionImgContainer"></div>')

              if (floatValue != 'none') {
                $(this).after('<p style="width: ' + maxWidth
                        + 'px; clear:both; float:' + floatValue
                        + '" class="imgCaption">' + imgtitle
                        + '</p>');
              } else {
                $(this).after(
                    '<p class="imgCaption">' + imgtitle + '</p>');
              }
            }
    ...
    /* Responsive blog iframes */
    $('.blogleft').find('iframe').each(function() {
      $(this).parent().addClass('iframeContainer');
      $(this).addClass('iframeBlog')
    });

    $('.sidebar').find('.blog-block').hover(function() {
      $(this).find('.blog-link').css('left', '0px');
    }, function() {
      $(this).find('.blog-link').css('left', '-200px');
    });

    /* display captcha on click */
    $('.blogleft').find('.blog-node-comments input').focus(function() {
      $('.blogleft').find('.captcha').css({
        'max-height': '400px',
        'overflow': 'auto',
        'display': 'block'
      });
    ...
    /* hide success message if ( $( '.blogCommentSuccess' ).length ) { */
    if ($('.blogPublishedDate').length) {
      $('p.blogCommentSuccess').click(function() {
        $('p.blogCommentSuccess').delay(50).fadeOut(1000);
      });
    }

    /* Fixed back to top div on small device */
    function fixDiv() {
      var $cache = $('.btp');
      var windowHeight = $('.blogHeadContain').height();
    ...
    // The id of the form.
    var newsInsights = $('#views-exposed-form-news-insights-page');
    if (newsInsights.length) {

      var timeoutID;
      // Submit the form after a short delay, to allow multiple changes
      // on each refresh.
      var delayedSubmit = function() {
        ...
        timeoutID = setTimeout(function() {
          var query = newsInsights.serialize();
          if (query == '') {
            // We need to change to something! This is better than ending with
          ...
          // We should be able to call the following line to refresh the view,
          // but due to a bug, it goes to the last pager link.
          // See https://www.drupal.org/project/views/issues/2199049, but
          // the fix there doesn't work here.
          // Instead we rely on this view being automatically submitted when
          // a field changes. :(
          //$('.view-news-insights').trigger('RefreshView');
          $('#edit-topic').trigger('change');
        }, 300);
      };
      
      // If the user clicks back or forward, the page needs to be be refreshed
      // somehow. This is the simplest way.
      ...
      // The count of all the currently viewed terms. If 0, hide the currently
      // viewing message (currentViewContainer).
      var currentViewCount = 0;

      // This container holds all our elements, for themeing purposes.
      var controls = $('<div class="news-insights-controls"></div>');
      // This is the container that holds all the currently viewed options.
      // (all are loaded, and hidden or shown as required)
      var currentViewNode = document.createElement('div');
      var currentlyViewing = $(currentViewNode).on('click', 'span', function() {
        var option = $(this).data('news-insights');
        if (option.selected) {
      ...
      var headingsContainer = document.createElement('div');
      headingsContainer.className = 'term-headings-container';
      controls.append(headingsContainer);
      
      var termsContainer = document.createElement('div');
      termsContainer.className = 'terms-container';
      controls.append(termsContainer);
      
      var currentViewContainer = document.createElement('div');
      currentViewContainer.innerHTML = '<p class="currently-viewing">Currently viewing:</p>';
      currentViewContainer.className = 'dropdown';
      currentViewContainer.appendChild(currentViewNode);
      
      // Utility function, update the display of a term in the currently
      // viewed list and the dropdown associated with the provided option.
      // If no terms are shown, hide the 'currently viewing' message.
      var showState = function(option) {
        var items = $(option).data('news-insights');
        if (option.selected) {
          $(items.drop).addClass('selected');
          $(items.view).addClass('term-open');
          $(items.view).removeClass('term-close');
        }
        else {
          $(items.drop).removeClass('selected');
          $(items.view).addClass('term-close');
          $(items.view).removeClass('term-open');
        }
        ...
        if (currentViewCount > 0) {
          ...
          $(currentViewContainer).hide();
        }
      };

      // dropdown is the container holding all the topic options.
      var termSets = [{
          name: 'Categories',
          selector: 'select#edit-category',
          dropdown: document.createElement('div')
        }, {
          name: 'Topics',
          selector: 'select#edit-topic',
          dropdown: document.createElement('div')
        }
      ];

      var filterTitle = document.createElement('h2');
      filterTitle.innerHTML = 'Filter by:';
      headingsContainer.appendChild(filterTitle);

      var viewAll = document.createElement('div');
      viewAll.innerHTML = 'All stories';

      // Drupal AJAX remembers the original location, so empty filters get
      // processed as if they had the URL parameters set. If there are URL
      // parameters set, the All stories button reloads the page to clear the
      // Drupal AJAX state.
      if (window.location.search == '') {
        $(viewAll).on('click', function() {
          for (var i in termSets) {
            $(termSets[i].dropdown).hide();
            currentViewCount = 0;
            $(termSets[i].selector + ' option', newsInsights).each(function() {
              this.selected = false;
              showState(this);
            });
            delayedSubmit();
      ...
      headingsContainer.appendChild(viewAll);

      for (var i in termSets) {
        var dropdown = termSets[i].dropdown;
        dropdown.className = 'term-group';
        
        $(termSets[i].selector + ' option', newsInsights).each(function() {
          var dropItem = document.createElement('span');
          var viewItem = document.createElement('span');

          dropItem.innerHTML = viewItem.innerHTML = this.innerHTML;

          dropItem.className = 'term available';
          viewItem.className = 'term selected-term';

          $(dropItem).data('news-insights', this);
          $(viewItem).data('news-insights', this);

          $(this).data('news-insights', {
            drop: dropItem,
            view: viewItem
          });
          ...
          currentlyViewing.append(viewItem);
          dropdown.appendChild(dropItem);
        });

        $(dropdown).on('click', 'span', function() {
          var option = $(this).data('news-insights');
          currentViewCount += option.selected ? -1 : 1;
        ...
        dropTitle.innerHTML = termSets[i].name;
        dropTitle.className = 'drop-arrow';

        // This section assumes exactly two dropdowns. 
        (function(dropdown, other) {
          $(dropTitle).on('click', function() {
            $(other).hide();
            $(dropdown).toggle();
          });
        })(dropdown, termSets[1 - i].dropdown);

        headingsContainer.appendChild(dropTitle);
        termsContainer.appendChild(dropdown);
      }

      controls.append(currentViewContainer);
      newsInsights.before(controls).hide();
      
      $('.term-headings-container > div').on('click', function() {
        $('.term-headings-container > div').removeClass('selected');
